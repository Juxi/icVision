	...
	if(img != NULL) {
		...
		if(img->depth == 32) m_leftImage = img;
			...
			if(m_leftImage == NULL) printError("The left image could not be converted to a 32bit image)!");
				...
				char help[200];
				sprintf_s(help, "The left image was set.\twidth: %d height:%d", m_leftImage->width, m_leftImage->height);
				printDebug(help);
			}
		}
	}
	...
	if(img != NULL){
		...
		if(img->depth == 32) m_rightImage = img;
			...
			if(m_rightImage == NULL) printError("The right image could not be converted to a 32bit image)!");
				...
				char help[200];
				sprintf_s(help, "The right image was set.\twidth: %d height:%d", m_rightImage->width, m_rightImage->height);
				printDebug(help);
			}
		}
	}
...
void DisparityMapper::loadLeftImage(char *file) {
	...
	if(!help) {
		char help[200];
		sprintf_s(help, "Could not load image file: %s", file);
		printError(help);
	} else 
		setLeftImage(help);
}
...
void DisparityMapper::setShiftingRange(int horizStart, int horizEnd, int horizStep,
	...
	char str[99];
	sprintf_s(str, "Shifing Range set to (%d, %d, %d)/(%d, %d, %d)!", 
...
void DisparityMapper::setLikelihoodParameters(char *distribution, double sigma) {
	...
	char str[99];
	sprintf_s(str, "Likelihood parameter set to %f", m_likelihoodSigma);
	printDebug(str);
}
