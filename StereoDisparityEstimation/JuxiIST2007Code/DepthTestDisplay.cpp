...
DepthTestDisplay::DepthTestDisplay(void)
{
	...
	strcpy_s(m_mainWindowName, "Main Window" );
	// get desktop sizes (maybe later?)
	int width = 800, height = 950;
	cvNamedWindow(m_mainWindowName, 0);
	cvResizeWindow(m_mainWindowName, width, height);

	m_dispImage = cvCreateImage( cvSize(width, height), 8, 3 );
	cvShowImage(m_mainWindowName, m_dispImage);
}
...
void DepthTestDisplay::setDiffImages(IplImage **images, int cnt) {
	...
	// load all pictures
	m_nImageCount = cnt;
	m_nDisplayImageIndex = max((int)cnt/2-1, 0);
...
void DepthTestDisplay::updateDisplay(void){
	...
	if(m_nImageCount > 0) {
		...
		char help[20];
		int coY = 0, coX = 0;
		for(int i = 0;i < min(3, m_nImageCount);i++) {
			if(k > endk) {
				k = -endk;
				l++;
			}
			...
			if(m_nImageCount < 3)
				...
				coX += gap;
			// put text
			sprintf_s(help, "%2d/%2d", k*stepWidth, l*stepWidth);
...
DepthTestDisplay::~DepthTestDisplay(void) {
	...
	cvDestroyWindow(m_mainWindowName);
}
