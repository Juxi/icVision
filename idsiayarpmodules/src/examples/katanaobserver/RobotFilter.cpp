...
static int getNumberOfAxes(std::string remoteBaseName) {
    ...
    std::cout << "  number of axes on robot: " << numberOfAxes << std::endl;
...
bool RobotFilter::open(RobotInterface &robot) {
	...
	for (int branch = 0; branch < robot.getNumBranches(); branch++) {
		...
		if (true == p_cbf->open(filterName.toStdString().c_str(),
			...
			// create and set command observer
			p_cmdo = new CommandObserver(robot, branch);
			p_cbf->setCommandObserver(p_cmdo);
			observers.append(p_cmdo);

			std::cout << "  CommandObserver set" << std::endl;
