#include "sphere.h"
#include "displmatrix.h"
#include <qgl.h>
#include <iostream>

using namespace std;

Sphere::Sphere( qreal r, const QVector3D& pos, int detail ) : lod(detail), radius(r)
{
    // make a sphere
    dtCreateObject( this, dtSphere( radius ) );
    dtLoadIdentity();

    // put it where it wants to be in the Local ObjectCS
    setL( DisplMatrix(QVector3D(),pos,0,pos.length()) );
}
Sphere::~Sphere()
{
}
void Sphere::makeDisplayList() {
    float X=radius*0.525731112119133606,
          Z=radius*0.850650808352039932;

    static GLfloat vdata[12][3] = {
        {-X, 0, Z}, { X, 0,  Z}, {-X,  0, -Z}, { X,  0, -Z},
        { 0, Z, X}, { 0, Z, -X}, { 0, -Z,  X}, { 0, -Z, -X},
        { Z, X, 0}, {-Z, X,  0}, { Z, -X,  0}, {-Z, -X,  0},
    };

    static GLint tindices[20][3] = {
        {0,4,1}, {0,9,4}, {9,5,4}, {4,5,8}, {4,8,1},
        {8,10,1}, {8,3,10}, {5,3,8}, {5,2,3}, {2,7,3},
        {7,10,3}, {7,6,10}, {7,11,6}, {11,0,6}, {0,1,6},
        {6,1,10}, {9,0,11}, {9,11,2}, {9,2,5}, {7,2,11}
    };

    // get a unique display list index and define the list
    setDisplayListIdx(glGenLists(10000));  // (this number determines the max number of physical objects representable in OpenGL)

    glNewList( displayListIdx(), GL_COMPILE );  // a cylinder expressed in a centroidal y-principal CS

        for ( int i=0; i<20; i++ ) {
            subdivide(&vdata[tindices[i][0]][0],
                      &vdata[tindices[i][1]][0],
                      &vdata[tindices[i][2]][0],
                      lod);
        }

        /*qreal csLen = 1.5*radius;
        GLfloat red[] = {1,0,0,1};
        GLfloat green[] = {0,1,0,1};
        GLfloat blue[] = {0,0,1,1};
        glBegin(GL_LINES);
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, red);
            glMaterialfv( GL_FRONT_AND_BACK, GL_SPECULAR, red);
            glVertex3f(0,0,0); glVertex3f(csLen,0,0);

            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, green);
            glMaterialfv( GL_FRONT_AND_BACK, GL_SPECULAR, green);
            glVertex3f(0,0,0); glVertex3f(0,csLen,0);

            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, blue);
            glMaterialfv( GL_FRONT_AND_BACK, GL_SPECULAR, blue);
            glVertex3f(0,0,0); glVertex3f(0,0,csLen);
        glEnd();*/

    glEndList();
    cout << "made sphere display list" << endl;
}
void Sphere::drawTriangle( float* v1, float* v2, float* v3 )
{
    glBegin(GL_TRIANGLES);
        glNormal3fv(v1); glVertex3fv(v1);
        glNormal3fv(v2); glVertex3fv(v2);
        glNormal3fv(v3); glVertex3fv(v3);
    glEnd();
}
void Sphere::subdivide( float* v1, float* v2, float* v3, int depth )
{
    float v12[3], v23[3], v31[3];
    int i;

    if (depth == 0) {
            drawTriangle(v1, v2, v3);
            return;
    }
    for (i = 0; i < 3; i++) {
            v12[i] = v1[i]+v2[i];
            v23[i] = v2[i]+v3[i];
            v31[i] = v3[i]+v1[i];
    }
    normalize(v12);
    normalize(v23);
    normalize(v31);
    subdivide(v1, v12, v31, depth - 1);
    subdivide(v2, v23, v12, depth - 1);
    subdivide(v3, v31, v23, depth - 1);
    subdivide(v12, v23, v31, depth - 1);
}
void Sphere::normalize(float v[3]) {
    float d = sqrt(v[0]*v[0]+v[1]*v[1]+v[2]*v[2]);
    if (d != 0) {
        v[0] = radius*v[0]/d;
        v[1] = radius*v[1]/d;
        v[2] = radius*v[2]/d;
    }
}


