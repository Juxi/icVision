...
void Blob::joinWithBlob(const Blob& blob)
{
	...
	{
		last_row_start_col = minimum(blob.last_row_start_col,last_row_start_col);
...
bool Blob::isValid(const DotParam& param) const
{
	return (pixels<=param.pixelsMax && 
			pixels>=param.pixelsMin && 
			max_col-min_col+1<=param.sizeMax && 
			max_col-min_col+1>=param.sizeMin && 
			max_row-min_row+1<=param.sizeMax && 
