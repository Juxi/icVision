...
ReflexFilter::ReflexFilter( KinematicModel::Robot* r, bool visualize ) : MoBeE::RobotFilter( r, visualize) 
{
}
...
void ReflexFilter::collisionResponse()
{
	...
	QVector< QVector< MoBeE::CircularBuffer::Item > > history;
	qreal period = 0;
	for ( int bodyPart = 0; bodyPart < robot->numBodyParts(); bodyPart++)
	{
	...
	for ( int i = 0; i < POSE_BUFFER_SIZE ; i++ )
	{
		...
		if ( wpReached ) { 
			...
			for ( int bodyPart = 0; bodyPart < robot->numBodyParts(); bodyPart++ )
				wpReached = wpReached || (history.at(bodyPart).at(i).label == MoBeE::StateObserver::WAYPOINT); // at least one bodypart reaches the waypoint
...
{
	MoBeE::CircularBuffer::Item currentPose;
