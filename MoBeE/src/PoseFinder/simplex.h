...
public:
	Simplex(Function& fit) : f(fit) {};
	Function &f;

	template<class D>
	std::vector<D> run(std::vector<D> init,    //initial guess of the parameters
	...
	{    //iteration step number
		...
		{ //construct the trial simplex
			...
			std::transform(del.begin(), del.end(), del.begin(), std::bind2nd( std::divides<D>() , 20) );//'20' is picked 
		...
		for(cnt=0; cnt<iterations; ++cnt){
			...
			D fxr=f.eval(xr);//record function at xr
		...
		if(cnt==iterations){//max number of iteration achieves before tol is satisfied
			std::cout<<"Iteration limit " << iterations << " achieved, result (" << vf[x1] << ") may not be optimal (" << tol << ")" <<std::endl;
		}
		return x[x1];
	}
};
