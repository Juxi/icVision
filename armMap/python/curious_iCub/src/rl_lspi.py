...
from PyBrainFix import LinFALearner
...
class RLThread(QThread):
    ...
    def init(self):
        ...
        controller = LSPI(self.nActions, self.nRLStates)
        self.trainAgent = LinearFA_Agent(controller)
        self.trainAgent.batchMode = True
        self.trainLearner = LinFALearner()
        ...
        for iController in xrange(self.nControllers):
            ...
            self.controllers[iController] = LSPI(self.nActions, self.nRLStates)
        
        # test modules
        self.testTask = intrinsicMotivationTask(self.environment)
        self.testTask.epiLen = 6
        self.testAgent = LinearFA_Agent(controller)
        self.testAgent.batchMode = True
    ...
    def run(self):
        ...
        for iEpisode in xrange(200):
            ...
            if ((pController != (self.nControllers-1)) and (nController != (self.nControllers-1)) and (pController != nController) ):
                ...
                untrainHistory.setField('state', np.atleast_2d(np.argmax(untrainHistory['state'],1)).T)  # fake quantization
                self.models[pController].addSamples(untrainHistory, np.array([np.argmax(finalState)]))

                # train on generated history
                #self.setHistory(self.trainAgent, self.models[pController].genSamples(1000))
                #self.trainAgent.learn(1)
                self.trainLearner.learnOnDataset(self.trainAgent.learner, self.models[pController].genSamples(1000, vector=True), repeat=10)
            ...
            if nController < (self.nControllers-1): # the final module is for random exploration, so will not be trained    
                ...
                trainHistory.setField('state', np.atleast_2d(np.argmax(trainHistory['state'],1)).T) # fake quantization
                self.models[nController].addSamples(trainHistory, np.array([np.argmax(finalState)]))
                
                # train on generated history
                #self.setHistory(self.trainAgent, self.models[nController].genSamples(1000))
                #self.trainAgent.learn(1)
                self.trainLearner.learnOnDataset(self.trainAgent.learner, self.models[nController].genSamples(1000, vector=True), repeat=10)
