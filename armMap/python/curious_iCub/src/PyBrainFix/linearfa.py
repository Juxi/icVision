...
class LSPImodel(LinearFALearner):
    """ LSPI with a model (Mark's version) """
    ...
    def _updateWeights(self, state, action, ptrans, rewards, next_states, next_actions):
        phi = zeros((self.num_actions, self.num_features))
        phi[action] += state        
        phi = ravel(phi)
        
        sum_phi_n = zeros((self.num_actions, self.num_features))
        sum_reward = 0
        for i in xrange(len(ptrans)):
            sum_phi_n[next_actions[i]] += ptrans[i] * next_states[i]
            sum_reward += ptrans[i] * rewards[i] 
        sum_phi_n = ravel(sum_phi_n)
    
        self._A += outer(phi, phi - self.rewardDiscount * sum_phi_n)
        self._b += sum_reward * phi
