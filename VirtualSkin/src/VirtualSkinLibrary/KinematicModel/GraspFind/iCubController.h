...
private:
	...
	PartController torso, left_arm, right_arm, head;
	...
	{ 
		...
		for ( int i=0; (unsigned int)i<data.size(); i++ )
		{
			...
			else if ( i < torso.getNumJoints() + head.getNumJoints() ) { right_armData.push_back( data.at(i) ); }

			else if ( i < torso.getNumJoints() + head.getNumJoints() + right_arm.getNumJoints() ) { right_armData.push_back( data.at(i) ); }
			else if ( i < torso.getNumJoints() + head.getNumJoints() + right_arm.getNumJoints() + left_arm.getNumJoints() ) { left_armData.push_back( data.at(i) ); }
		}
		return 1;
	}
};
