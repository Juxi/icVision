...
MoverMinJerkLinear::MoverMinJerkLinear( ) {
...
bool MoverMinJerkLinear::init(string& robot, vector<string>& parts ) {
	...
	dd.clear(); encs.clear(); lims.clear(); vels.clear(); vctrls.clear();
	partnames = parts;
	...
	// get and set some useful values, construct minjerk velocity controllers
	...
	for (int ipart=0; ipart<nparts; ipart++) {
		...
		vctrls.push_back(new minJerkVelCtrl(TS,nJoints[ipart]));
	}
...
bool MoverMinJerkLinear::connectFace(string& robot, string& rawFacePort) {
...
bool MoverMinJerkLinear::connnectVSkin(string& robot, string& portRpc) {
	bool success = true;
...
bool MoverMinJerkLinear::checkValidity() {
	for (int i=0; i<nparts; i++) {
...
void MoverMinJerkLinear::getMask(Bottle &b) {
	Bottle yParts;
	for (int ipart=0; ipart<nparts; ipart++) {
		Bottle yPart;
		for (int j=0; j<nJoints[ipart]; j++) {
...
void MoverMinJerkLinear::getPose(Bottle &b) {
	Bottle yParts;
	for (int ipart=0; ipart<nparts; ipart++) {
...
void MoverMinJerkLinear::maskFingers() {
	for (int ipart=0; ipart<nparts; ipart++) {
...
void MoverMinJerkLinear::maskEyes() {
	for (int ipart=0; ipart<nparts; ipart++) {
...
void MoverMinJerkLinear::maskHead() {
	for (int ipart=0; ipart<nparts; ipart++) {
		if ((partnames[ipart].find("head") >= 0) && (nJoints[ipart] == 6)) {
			for (int j=0; j<6; j++) {
				mask[ipart][j] = true;
...
void MoverMinJerkLinear::maskOff() {
	mask.clear();
	for (int ipart=0; ipart<nparts; ipart++) {
		mask.push_back(vector<bool>(nJoints[ipart]));
	}
...
bool MoverMinJerkLinear::setMask(Value &v) {
	Bottle* yParts;
	Bottle* yPart;
...
bool MoverMinJerkLinear::parseTrajBottle(Value &v, vector<vector<vector<double> > >& poses) {
	poses.clear();
...
void MoverMinJerkLinear::setWayPoint() {
...
void MoverMinJerkLinear::sendFace(vector<string> values) {
...
void MoverMinJerkLinear::monDone(bool reached) {
	monIndex = 0;
	if (reached)
		sendFace(monDoneVal);
	else
...
void MoverMinJerkLinear::monMoving(){
...
void MoverMinJerkLinear::monReflexing(){
	monIndex = 0;
	blink();
	sendFace(monReflVal);
}
