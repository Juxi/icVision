...
public:
	...
	void disarm() { armed = false; }	//!< When disarmed, the collisionDetector will not emit the crash() signal regardless
	bool isArmed() { return armed; }								//!< Returns whether or not the collisionDetector is armed
	...
	static void collisionHandler( void * client_data, DtObjectRef obj1, DtObjectRef obj2, const DtCollData *coll_data )
	{
		...
		yarp::os::Bottle& collision = detector->bottle.addList();
...
private:
    ...
    bool    armed;		//!< Whether or not to report collisions
