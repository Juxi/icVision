...
HeadController::HeadController( qreal _attenuationThreshold, qreal _attenuationFactor ) : 
number_of_joints(NUMBEROFHEADJOINTS),
...
void HeadController::addControlPoint(float a[NUMBEROFHEADJOINTS]) {
	ControlPoint next;
...
void HeadController::connect( const char* _robotName, const char* _partName, int refAccel, int refSpeed )
{
	...
	{
		QString err = QString("Wrong number of DOFs: HeadController must connect to an arm of the iCub robot, which has %1 joints. The requested device ").arg(number_of_joints);
	...
	printf("HeadController started successfully.\n");
}

void HeadController::checkValidity()
{
...
void HeadController::initialPose()
{
...
void HeadController::blockingPositionMove( ControlPoint* target )
{
	bool ok;
	for (int i = 0; i < number_of_joints; i++ )
	{
...
qreal HeadController::doVelocityControl( int speed, bool stopOnTouch )
{
	char buf[1024];
