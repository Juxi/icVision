...
bool VisionModule::updateModule()
{
  ...
  //Use YARP port to get images from cameras
  getImageLeft =  ( imageInLeft.getInputCount() > 0 );
  getImageRight = ( imageInRight.getInputCount() > 0 );

  //if one of the images is not available then skip the frame
  ...
  //Initialize bottle for motor readings
  ...
  if (receivedHead && receivedTorso)
   {
      ...
      //initialize the rotation and the translation of the camera wrt the world obtained from the motors
  ...
  //If both the images are available and also the reading from the motors
  if ( getImageLeft + getImageRight > 1 && receivedHead && receivedTorso){
    ...
    //Trasform the image as Mat value (see OpenCV for details)
   ...
   // outputImageRight = imageRight;
    ...
    //if the output image are initialized it is possible to send this information using the YARP ports
    ...
    //If you want to send data to the simulator
    if(sendData2Sim){

      //Iterate the object list
      for(uint i=0; i<object_list.size(); i++){

          //Count the number of time that an object is not changed, if this number is less than a threshold than you can send it
