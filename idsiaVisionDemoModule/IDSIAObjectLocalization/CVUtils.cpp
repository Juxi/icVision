...
//Estimate the fundamental Matrix from cameras parameter intrinsics (camera_left and camera_right)
//and the rotation R and the translation T between the two cameras
//For more details see Multiple View Geometry
...
//Estimate the fundamental Matrix from images using the intrinsic parameters of the cameras (camera_left and camera_right)
//and a pattern, here a chessboard 8x6
//For more details see Multiple View Geometry and OpenCV examples
void CVUtils::fundfromimages(Mat &camera_left, Mat &camera_right){
  ...
  //if you found the chessboard on the left and right cameras
 ...
 //   cout<<cameraMatrix_left.type()<<endl;
    ...
    //calibrate the camera (see OpenCV for more details)
...
//    cout<<cameraMatrix_left.type()<<endl;
    ...
    //Solve PnP problem to estimate the camera position w.r.t. the chessboard
    ...
    //Estimate the Rodrigues parameters
...
//Triangulate point to estimate the 3D point from the 2D correspondent points (see Bouguet calibration tool for more details)
...
//Undistort a 2D point removing the lens distortion. dist contains the parameters of the distortion
...
//Segment the image on the base of a color represented as mean m and inverse of covariance icov
void CVUtils::colorSegmentation(Mat &image, Mat &m, Mat &icov, Mat &binaryImage){
  ...
  //For each pixel estimate the Mahalanobis distance
  ...
  Mat dst;
  //If the malanobis distance is greater than 0.027 than this is an object and the result is a binary image
  compare(resultingImage, 0.027, binaryImage, CMP_LE); //0.03


  //Perform a filering of the binary image using a morphological operator
...
//Find possible colored object, putting this possible object in a list of colored rectangle
void CVUtils::detectPossibleColoredObject(Mat &image, Mat& outputMask, vector<ColoredRect> &listofrect, Mat& outputImage){
  ...
  //parse the statistic of the colors
  int boxcounter = 0;
  for(uint cc = 0; cc<colorsStat.size(); cc+=2){
    ...
    //in the recorded file: the first color is RED, the second is BLUE and the third is GREEN
    switch (cc) {
    ...
    //Segment the image on the base of the color
  ...
  // bitwise_or(outputMask, binaryImage, outputMask);
    ...
    //find the blob corresponding to the colored object
    findContours( binaryImage, contours_obj, hierarchy_obj, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE );

    //check the size of this blob to remove the noise
    if( contours_obj.size() > 0 )
    {
       ...
       for(int idx = 0; idx >= 0; idx = hierarchy_obj[idx][0] )
       {
          ...
          //Check the dimension of the bounding box
          if(objRect.area() > 650){
...
//Refine the matches to remove outliers considering the epipolar geometry
...
//Estimate the disparity map (see OpenCV examples)
...
// Perform data association beween the list of object and the left and right images
int CVUtils::dataAssociation(vector<ColoredRect> &left_rectList, vector<ColoredRect> &right_rectList, vector<WorldObject> &object_list){
  ...
  for(uint i = 0; i<left_rectList.size(); i++){
      ...
      //Iterate the list of object in the right image
 ...
 // object_list.clear(); // TODO remove it!
  ...
  for(uint i = 0; i<blobs_matches.size(); i++){
      ...
      //Select the left and the right rectangle
      ...
      //estimtate the difference betweent the two size
     ...
     // triangulatePoint(massCenter_right, massCenter_left, point3dRight, point3dLeft);
      ...
      //This is the pose wrt the left camera
      Point3f poseWrtLeft;
      ...
      //homogenoues version of the point
      ...
      //Rotate the point to estimate its position wrt the world
      rotatedpoint3Dpoint = RT_left*hompoint3dLeft;
      // cout<<rotatedpoint3Dpoint<<endl;

      //Estimate the point wrt the world refernece frame
      ...
      //The position of the 3D point must be in front of the camera
...
//initialize the transformation from camera to world
...
int CVUtils::detectObjects(vector<WorldObject> &obj_list){
  ...
  for(uint oo = 0; oo<obj_list.size(); oo++){
      ...
      //LEFT IMAGE
      //list of object in the left image
      left<<numberobj;
...
//Find object contours given a subimage of the current image
...
//Triangulate point using opencv (see OpenCV example)
