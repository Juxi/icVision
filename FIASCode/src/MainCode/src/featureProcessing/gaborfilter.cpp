...
void GaborFilter::setParam(unsigned int L, unsigned int D, float f, float kmax, float sigma, float N, std::vector<int>* filtersize)
{
	...
	if(!filtersize) { 
		#if DEBUGGABORFILTER
			std::cout<<"\n"<< nameclass << "::setParam::filtersize = " << filtersize;
		#endif	
	...
	#if DEBUGGABORFILTER
		std::cout<<"\n"<< nameclass << "::setParam::mvec_filtersize.size() = " << mvec_filtersize.size() << "\n";
		std::cout<<"\n"<< nameclass << "::setParam::m_L = " << m_L << "\n";
	#endif

	if(mvec_filtersize.size() != m_L) 
	{ 
