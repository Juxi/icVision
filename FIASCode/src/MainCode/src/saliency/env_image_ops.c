...
#include "iCub/saliency/env_image_ops.h"

#include "iCub/saliency/env_c_math_ops.h"
#include "iCub/saliency/env_log.h"
#include "iCub/saliency/env_params.h" //REV: added this for the envp in rgby...
...
//REV: modified so it takes envp too (for biases)
// ######################################################################
        ...
        )
{
        ...
        for (env_size_t i = 0; i < sz; ++i)
        {
                ...
                if (src2 == 0)
                { 
//REV: SWITCHED FOR OPENCV! (rgb->bgr order)(I think this is the only place...)
                        r = (intg32) src[i].p[2]; //0
                        g = (intg32) src[i].p[1]; //1
                        b = (intg32) src[i].p[0]; //2
                }
                else
                {
                        r = ((intg32) src[i].p[2] + (intg32) src2[i].p[2]) / 2;
                        g = ((intg32) src[i].p[1] + (intg32) src2[i].p[1]) / 2;
                        b = ((intg32) src[i].p[0] + (intg32) src2[i].p[0]) / 2;
                }
                ...
                else
                {
                        ...
                        intg32 red = (2*r - g - b) * (float)(envp->red_bias);
                        intg32 green = (2*g - r - b) * (float)(envp->green_bias);
                        intg32 blue = (2*b - r - g);
                        intg32 yellow = (-2*blue - 4*ENV_ABS(r-g)) * (float)(envp->yellow_bias);
                        blue = blue * (float)(envp->blue_bias);//had to add this since blue is used...
...
//REV: modified rgby extractor to take color biases...
// ######################################################################
void env_get_rgby_bias(const struct env_rgb_pixel* const src,
                  const struct env_rgb_pixel* const src2,
                  const env_size_t sz,
                  struct env_image* rg,
                  struct env_image* by, const intg32 thresh,
                  const env_size_t inputbits,
                  float red_bias, float green_bias, float blue_bias, float yellow_bias)
{
        // red = [r - (g+b)/2]        [.] = clamp between 0 and 255
        // green = [g - (r+b)/2]
        // blue = [b - (r+g)/2]
        ...
        for (env_size_t i = 0; i < sz; ++i)
        {
                ...
                if (src2 == 0)
                {
                        r = (intg32) src[i].p[2]; //REV: modified for opencv order...
                        g = (intg32) src[i].p[1];
                        b = (intg32) src[i].p[0];
                }
                else
                ...
                {
                        // now compute color opponencies:
                        intg32 red = (2*r - g - b) * red_bias;
                        intg32 green = (2*g - r - b) * green_bias;
                        intg32 blue = (2*b - r - g) * blue_bias;
                        intg32 yellow = (-2*blue - 4*ENV_ABS(r-g)) * yellow_bias;
