                    ...
                    struct env_image* result)
{
        ...
        env_get_rgby(colimg, prev_colimg, dims.w * dims.h,
                     &rg, &by, lumthresh, imath->nbits, envp);
...
//REV: changed funct to take color biases
// ######################################################################
void env_chan_color_bias(const char* tagName,
                    const struct env_params* envp,
                    const struct env_math* imath,
                    const struct env_rgb_pixel* const colimg,
                    const struct env_rgb_pixel* const prev_colimg,
                    ...
                    float red_bias, float green_bias, float blue_bias, float yellow_bias)
{
        ...
        //REV: modified to bias colors...
        env_get_rgby_bias(colimg, prev_colimg, dims.w * dims.h,
                     &rg, &by, lumthresh, imath->nbits, red_bias, green_bias, blue_bias, yellow_bias);
