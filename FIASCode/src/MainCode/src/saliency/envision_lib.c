...
#include "iCub/saliency/env_alloc.h"
#include "iCub/saliency/env_c_math_ops.h"
#include "iCub/saliency/env_image.h"
#include "iCub/saliency/env_image_ops.h"
#include "iCub/saliency/env_log.h"
#include "iCub/saliency/env_mt_visual_cortex.h"
#include "iCub/saliency/env_params.h"
#include "iCub/saliency/env_pthread_interface.h"
#include "iCub/saliency/env_stdio_interface.h"
#include "iCub/saliency/env_visual_cortex.h"
...
int SC_subset_winner(int* l_subset, int l_subset_l, int* r_subset, int r_subset_l) //assume l NUMBER OF POINTS (not size of array)
{
  ...
  for(int x=0; x<l_subset_l; x+=2)
    {
      int xloc = (int)(l_subset[x] / 16); //have to div by 16
		//printf("\nxlox[%d] = %d",x,xloc);
      int yloc = (int)(l_subset[x+1] / 16); //have to div by 16
		//printf("\nylox[%d] = %d",x,yloc);
  ...
  for(int x=0; x<r_subset_l; x+=2)
    {
      int xloc = (int)(r_subset[x] /16);
		//printf("\nxlox[%d] = %d",x,xloc);      
      int yloc = (int)(r_subset[x+1] /16);
		//printf("\nylox[%d] = %d",x,yloc);      
