...
#include "iCub/saliency/env_channel.h"
#include "iCub/saliency/env_image_ops.h"
#include "iCub/saliency/env_job_server.h"
#include "iCub/saliency/env_log.h"
#include "iCub/saliency/env_mt_channel.h"
...
//REV: modified to do color bias
// ######################################################################
void env_mt_visual_cortex_input_bias(
        const int do_multithreaded,
        struct env_visual_cortex* vcx,
        const struct env_params* envp,
        const char* tagName,
        ...
        )
{
        if (!do_multithreaded)
        {
                env_visual_cortex_input_bias(
                        vcx, envp, tagName, colimg, prev_colimg, dims,
                        status_func, status_userdata,
                        result,
                        intens_result, color_result, red_bias, green_bias, blue_bias, yellow_bias,
                        ori_result
#ifdef ENV_WITH_DYNAMIC_CHANNELS
                        , flicker_result, motion_result
#endif
